...
export default async function AlumnosPage() {
  ...
  const { data: alumnos } = await supabase
    .from('perfiles')
    .select('id, nombre_completo, grado, usuario, celular, created_at, tipo_usuario, escuelas(nombre)')
    .eq('rol', 'alumno')
    .order('created_at', { ascending: false })

  return (
    <div>
      <h1 className="text-2xl font-black text-gray-800 mb-1">Usuarios registrados</h1>
      <p className="text-sm text-gray-500 mb-6">
        Total: <strong>{alumnos?.length ?? 0}</strong> usuarios
      </p>
              ...
              {(alumnos ?? []).map((a, i) => (
                <tr key={a.id} className={`border-t border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                  ...
                  <td className="py-3 px-4">
                    {a.tipo_usuario === 'maestro'
                      ? <span className="inline-block bg-purple-100 text-purple-700 text-xs font-bold px-2 py-0.5 rounded-full">Maestro</span>
                      : <span className="text-gray-400 text-xs">Alumno</span>
                    }
                  </td>
